...
public final class Bookmark extends ZLTextFixedPosition
{
	...
	{
		//--保存书签时，判断开始段落索引是不是上章的结束标记，如果是，向后移动一个段落索引，防止存储书签索引时，存成上章的书签。--
		boolean isKind = startCursor.getParagraphCursor().isEndOfSection();
		if(isKind){
		...
		if(startCursor.equals(buffer.Cursor)){
			// 边界问题
			return null;
		}else{
