...
public final class ConfigShadow extends Config implements ServiceConnection
{
	...
	// 远程服务连通之前放入的待执行任务，当连通时，遍历执行
	private final List<Runnable>		myDeferredActions			= new LinkedList<Runnable>();

	// 远程操作(更改配置(通过数据库，SharedPrefrence等))完成后发出的广播通知，这里注册一个广播接收
	private final BroadcastReceiver		myReceiver					= new BroadcastReceiver()
																	{
																		public void onReceive(Context context, Intent intent)
																		{
																			try {
																				// 缓存数据
																				setToCache(intent.getStringExtra("group"),
	...
	/**
	 * 构造ConfigShadow时便开启/绑定ConfigService服务
	 * 
	 * @param context
	 */
	...
	{
		myContext = context;
		context.bindService(FBReaderIntents
				.internalIntent(FBReaderIntents.Action.CONFIG_SERVICE), this,
	...
	{
		return myContext.getSharedPreferences("fbreader.ui",
				Context.MODE_PRIVATE).getBoolean(name, defaultValue);
	}
	...
	{
		return myContext.getSharedPreferences("fbreader.ui",
				Context.MODE_PRIVATE).getString(name, defaultValue);
	}
	...
	@Override
	protected String getValueInternal(String group, String name)
			throws NotAvailableException
	{
		if (myInterface == null) {
			throw new NotAvailableException("Config is not initialized for "
					+ group + ":" + name);
		}
		try {
	...
	@Override
	protected Map<String, String> requestAllValuesForGroupInternal(String group)
			throws NotAvailableException
	{
		if (myInterface == null) {
			throw new NotAvailableException("Config is not initialized for "
			...
			IBinder service)
	{
		...
		myContext.registerReceiver(myReceiver, new IntentFilter(
