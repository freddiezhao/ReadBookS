...
public abstract class UIUtil {
					...
					try {
						synchronized (ourMonitor) {
							if (ourTaskQueue.isEmpty()) {
								...
								ourProgress
										.setMessage(ourTaskQueue.peek().Message);
							}
							ourMonitor.notify();
						}
					} catch (Exception e) {
	...
	public static void wait(String key, Runnable action, Context context) {
		...
		synchronized (ourMonitor) {
			final String message = ZLResource.resource("dialog")
					.getResource("waitMessage").getResource(key).getValue();
			ourTaskQueue.offer(new Pair(action, message));
			if (ourProgress == null) {
				ourProgress = ProgressDialog.show(context, null, message, true,
						false);
			} else {
				return;
			}
	...
	public static ZLApplication.SynchronousExecutor createExecutor(
			final Activity activity, final String key) {// key:loadingBook
		return new ZLApplication.SynchronousExecutor() {
			private final ZLResource myResource = ZLResource.resource("dialog")
					.getResource("waitMessage");
			private final String myMessage = myResource.getResource(key)
					.getValue();
			private volatile ProgressDialog myProgress;

			public void execute(final Runnable action,
					final Runnable uiPostAction) {
				activity.runOnUiThread(new Runnable() {
					public void run() {
						// myProgress = ProgressDialog.show(activity, null,
						// myMessage, true, false);
						DialogUtils.showProgressDialog(activity,
								R.string.opening_file, true, null, null);
									...
									public void run() {
										try {
											...
											if ("loadingBook".equals(key))
												MaskGuideActivity
														.launch(activity,
																StorageUtil.KEY_SHOW_GUIDE_READ,
																R.layout.guide_read);
										} catch (Exception e) {
											e.printStackTrace();
										}
			...
			private void setMessage(final ProgressDialog progress,
					final String message) {
				if (progress == null) {
					return;
				}
	...
	public static void showMessageText(final Activity activity,
			final String text) {
		activity.runOnUiThread(new Runnable() {
			public void run() {
	...
	public static void showErrorMessage(Activity activity, String resourceKey) {
		showMessageText(activity, ZLResource.resource("errorMessage")
				.getResource(resourceKey).getValue());
	}

	public static void showErrorMessage(Activity activity, String resourceKey,
			String parameter) {
		showMessageText(activity, ZLResource.resource("errorMessage")
				.getResource(resourceKey).getValue().replace("%s", parameter));
	}
}
