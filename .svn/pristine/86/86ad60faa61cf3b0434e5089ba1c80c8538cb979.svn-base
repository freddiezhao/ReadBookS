...
/**
 * 负责获取当前APP的一些参数，如程序版本号，手机屏幕宽高，亮度调节，电池电量，支持的语言版本等<br>
 * 还有一个内部类{@link AndroidAssetsFile} 代表assets下的资源。
 * 
 * @author chenjl
 * 
 */
public final class ZLAndroidLibrary extends ZLibrary
{
	...
	{
		int	UNKNOWN		= -1;	// 未知
		int	NULL		= 0;	// 为空，比如根本找不到这个文件
		int	OK			= 1;	// 流数据正常
		int	EXCEPTION	= 2;	// 流数据异常
	}
	...
	private final class AndroidAssetsFile extends ZLResourceFile
	{
		...
		protected List<ZLFile> directoryEntries()
		{
			try {
				...
				// 如果names长度不为0，说明是assets资源下的一个文件夹
		...
		/**
		 * 文件流状态<br>
		 * 通过调用{@link android.content.res.AssetManager#open(String)}方法来判断
		 */
