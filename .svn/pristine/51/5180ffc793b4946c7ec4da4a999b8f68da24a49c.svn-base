...
public class SlidingUpPanelLayout extends ViewGroup {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		// 当slide view 展开时，拦截slide view 外的事件
		if (isSlideViewExpand() && !isSlideableViewUnder((int) x, (int) y)) {
			return true;
		}
		...
		case MotionEvent.ACTION_UP: {
			...
			if (dx * dx + dy * dy < slop * slop) {
				if (isDragViewUnder((int) x, (int) y)) {
					...
					// 如果slide view展开且触摸slide view 外侧时收起slide view
					if (isSlideViewExpand() && !isSlideableViewUnder((int) x, (int) y)) {
						collapsePane();
					}
				}
	...
	private boolean isSlideableViewUnder(int x, int y) {
		View slideableView = mSlideableView;
		if (slideableView == null)
			return false;
		int[] viewLocation = new int[2];
		slideableView.getLocationOnScreen(viewLocation);
		int[] parentLocation = new int[2];
		this.getLocationOnScreen(parentLocation);
		int screenX = parentLocation[0] + x;
		int screenY = parentLocation[1] + y;
		return screenX >= viewLocation[0] && screenX < viewLocation[0] + slideableView.getWidth()
				&& screenY >= viewLocation[1] && screenY < viewLocation[1] + slideableView.getHeight();
	}
	...
	/**
	 * Check if the layout is completely expanded or anchored in an intermediate
	 * point.
	 * 
	 * @return true if sliding panels are completely expanded or anchored
	 */
	public boolean isSlideViewExpand() {
