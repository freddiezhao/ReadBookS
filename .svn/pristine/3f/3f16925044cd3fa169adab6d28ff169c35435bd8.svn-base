...
/**
 * 书籍数据库操作类<br>
 * 关于本类中的部分方法用到的一些SQL语句的解释：<br>
 * (1) INSERT OR IGNORE INTO TableName(?, ?) VALUES(?, ?) <br>
 * 当插入数据时，如出现错误时，如重复数据，将不返回错误，<br>
 * 只以警告形式返回。所以使用ignore请确保语句本身没有问题，<br>
 * 否则也会被忽略掉。
 * 
 * @author chenjl
 * 
...
final class SQLiteBooksDatabase extends BooksDatabase
{
	...
	private void createTables()
	{
		myDatabase.execSQL(
				"CREATE TABLE IF NOT EXISTS Books(" +
						"book_id INTEGER PRIMARY KEY," + // book_id作为主键
						"encoding TEXT," +
						"language TEXT," +
						"title TEXT NOT NULL," +
						"file_name TEXT UNIQUE NOT NULL)");// unique
															// 表示该列的值将必须唯一，不可重复
		myDatabase.execSQL(
		...
		// 为name,sort,key三个列添加约束条件，约束别名为Authors_Unique，约束条件是unique，唯一性约束
		myDatabase.execSQL(
				"CREATE TABLE IF NOT EXISTS BookAuthor(" +
						"author_id INTEGER NOT NULL REFERENCES Authors(author_id)," + // author_id要求非空并且值被引用自Authors表的author_id列
	...
	private void updateTables1()
	{
		...
		// 将从Tags_Obsolete表中查询出来的tag_id,name,parent三列的数据插入到Tags表中
	...
	{
		// create index 索引别名 on 表名(列名1,列名2,列名3...)
		// 在某个表的某个列上创建索引，创建索引能加快查找速度
