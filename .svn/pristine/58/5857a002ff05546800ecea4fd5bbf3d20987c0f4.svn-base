...
{
	private static int	animateSpeed	= 8;	// 动画速度，fbreader中在菜单设置,此处可直接修改该值为1<<10之间
	...
	void startAnimatedScrolling(int x, int y, float velocityX, float velocityY, int speed)
	{
		speed = animateSpeed;
		if (myMode != Mode.ManualScrolling) {
			return;
		...
		boolean isPendingToPrevious = getPageToScrollTo() == ZLView.PageIndex.previous;
		// 横向有速度
		if (Math.abs(velocityX) >= 1000 && !isPendingToPrevious) {
			if (velocityX > 0) {
				// 由左向右滑动 →
				forward = false;
			} else if (velocityX < 0) {
		...
		// Log.d("VelocityTracker", "startAnimatedScrolling -> minDiff=" +
		// minDiff + ", diff=" + diff + ", forward=" + forward +
	...
	public void startAnimatedScrolling(ZLView.PageIndex pageIndex, Integer x,
			Integer y, int speed)
	{
		speed = animateSpeed;
		if (myMode.Auto) {
