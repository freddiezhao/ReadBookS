 ...
 * 我的购买
 */
public class BuyDetailFragment extends BaseFragment implements IXScrollViewListener, ITaskFinishListener
{
	...
	private void requestData(int page)
	{
		...
		String url = String.format(Locale.CHINA, ConstantData.URL_CONSUME, page, "books_list");
		url = ConstantData.addLoginInfoToUrl(url);
		TaskParams params = new TaskParams();
		params.put(RequestTask.PARAM_URL, url);
		params.put(RequestTask.PARAM_HTTP_METHOD, RequestTask.HTTP_GET);
		RequestTask task = new RequestTask(new BuyDetailParser());
	...
	private void initView()
	{
		...
		if (null != content) {
			...
			mAdapter = new BuyDetailAdapter(getActivity());
	...
	public void onTaskFinished(TaskResult taskResult)
	{
		...
		if (result instanceof ListResult<?>) {
			...
			ListResult<BuyDetail> listResult = (ListResult<BuyDetail>) result;
