...
public class LogUtil {
	...
	public static String CX_TAG = "cx";
	public static String TAG_MJW = "mjw";

	private static String DEF_TAG = "SinaBook";
	 ...
	 * log 标志位，false关闭
	 */
	public static boolean gLogFlag = SinaBookApplication.isDebug();
	...
	public static void fileLogE(String msg) {
		writeLogInfoToFile(EXCEPTION_LOG_FILE_PATH, MAX_CRASHLOG_FILE_LENGTH, msg);
	}
	...
	private static void writeLogInfoToFile(final String filePath, final int logFileLength, final String logInfo) {
		try {
			if (StorageUtil.isSDCardExist()) {
				// make sure file created
				final File file = new File(StorageUtil.getDirByType(StorageUtil.DIR_TYPE_LOG), filePath);
				if (file.exists()) {
				...
				if (!file.exists()) {
					...
					// file.createNewFile();
					FileUtils.checkAndCreateFile(file.getAbsolutePath());
				}
