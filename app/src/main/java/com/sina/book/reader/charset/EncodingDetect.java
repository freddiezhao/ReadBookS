...
public class EncodingDetect {
    ...
    private static final int BIG5 = 2;
    private static final int EUC_TW = 3;
    private static final int ISO_2022_CN = 4;
    private static final int UTF8 = 5;
    private static final int UNICODE = 6;
    private static final int ASCII = 7;
    private static final int OTHER = 8;

    private static final int TOTAL_ENCODINGS = 9;

    /**
     * 默认的编码格式
     */
    public static final String CODDING_DEFAULT_GBK = "GBK";
    /**
     * 判断编码格式的字节数
     */
    private static final int BYTES_CHECK_LIMIT = 4096;
    ...
    public EncodingDetect() {
        ...
        // GBK兼容GB2312
        codings[GB2312] = CODDING_DEFAULT_GBK;
        codings[GBK] = CODDING_DEFAULT_GBK;
        codings[BIG5] = "BIG5";
        codings[EUC_TW] = "EUC-TW";
        codings[ISO_2022_CN] = "ISO2022CN";
        codings[UTF8] = "UTF-8";
        codings[UNICODE] = "Unicode";
        codings[ASCII] = "ASCII";
        codings[OTHER] = CODDING_DEFAULT_GBK;
    ...
    public String detectEncoding(File testfile) {
        if (testfile == null) {
            return CODDING_DEFAULT_GBK;
        }
        int checkLength = (int) testfile.length();
        if (checkLength > BYTES_CHECK_LIMIT) {
            checkLength = BYTES_CHECK_LIMIT;
        }

        FileInputStream chinesefile = null;
        byte[] rawtext;

        rawtext = new byte[checkLength];
        try {
        ...
        // BOM check
        if (rawtext.length >= 3) {
            if (rawtext[0] == (byte) 0xFF && rawtext[1] == (byte) 0xFE) {
                return "UTF-16LE";
            } else if (rawtext[0] == (byte) 0xFE && rawtext[1] == (byte) 0xFF) {
                return "UTF-16BE";
            } else if (rawtext[0] == (byte) 0xEF && rawtext[1] == (byte) 0xBB
                    && rawtext[2] == (byte) 0xBF) {
                return "UTF-8";
            }
        }
     ...
     */
    public String detectEncoding(byte[] rawtext) {
        int[] scores;
        ...
        return getEncodingName(encoding_guess);
    }
     ...
     * @return
     */
    public String detectMiniEncoding(File testfile) {
        if (testfile == null) {
            return CODDING_DEFAULT_GBK;
        }
        int checkLength = (int) testfile.length();
        ...
        // BOM check
        if (rawtext.length >= 3) {
            if (rawtext[0] == (byte) 0xEF && rawtext[1] == (byte) 0xBB
                    && rawtext[2] == (byte) 0xBF) {
                return "UTF-8";
            }
        }
        
        return detectMiniEncoding(rawtext);
    }
     ...
     * @return
     */
    public String detectMiniEncoding(byte[] rawtext) {
        int[] scores;
        int index, maxscore = 0;
        int encoding_guess = 3;

        int mini_encodings = 4;
        int gb2312 = 0;
        int gbk = 1;
        int utf8 = 2;
        int other = 3;

        scores = new int[mini_encodings];
        ...
        // Tabulate Scores
        for (index = 0; index < mini_encodings; index++) {
            if (scores[index] > maxscore) {
                encoding_guess = index;
                maxscore = scores[index];
            }
        ...
        // Return OTHER if nothing scored above 50
        if (maxscore <= 50 || encoding_guess != utf8) {
            return CODDING_DEFAULT_GBK;
        } else {
            return "UTF-8";
        }
    ...
    String getEncodingName(int type) {
        return codings[type];
    }
}