...
abstract class CachedCharStorageBase implements CharStorage {
	...
	public char[] block(int index) {
		...
		if (block == null) {
			try {
				File file = new File(fileName(index));// 缓存书籍的地方，如/storage/emulated/0/Android/data/com.sina.book/cache/0.ncache
				int size = (int)file.length();// 该文件由底层代码写入
				if (size < 0) {
					throw new CachedCharStorageException("Error during reading " + fileName(index));
				}
				block = new char[size / 2];// 分两部分存储在myArray中
