...
public class ZLTextNGStyleDescription {
	...
	// add by yq
	int getTopMargin(ZLTextMetrics metrics, int base, int fontSize) {
		final ZLTextStyleEntry.Length length = parseLength(MarginTopOption.getValue());
		if (length == null) {
			return base;
		}
		return base + ZLTextStyleEntry.compute(
			length, metrics, fontSize, ZLTextStyleEntry.Feature.LENGTH_MARGIN_TOP
		);
	}

	int getBottomMargin(ZLTextMetrics metrics, int base, int fontSize) {
		final ZLTextStyleEntry.Length length = parseLength(MarginBottomOption.getValue());
		if (length == null) {
			return base;
		}
		return ZLTextStyleEntry.compute(
			length, metrics, fontSize, ZLTextStyleEntry.Feature.LENGTH_MARGIN_BOTTOM
		);
	}
	
	int getTopPadding(ZLTextMetrics metrics, int base, int fontSize) {
		return base;
	}

	int getBottomPadding(ZLTextMetrics metrics, int base, int fontSize) {
		return base;
	}
