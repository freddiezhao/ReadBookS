...
abstract class ZLTextViewBase extends ZLView {
	...
	protected ZLTextMetrics metrics() {
		...
		// be returned from this method enen(甚至) in multi-thread environment
		// 这个局部变量用来保证此方法不会返回null，甚至是在多线程的环境中。
	...
	final int getWordHeight() {
		if (myWordHeight == -1) {
			...
			myWordHeight = getContext().getStringHeight()
					* textStyle.getLineSpacePercent() / 100
					+ textStyle.getVerticalAlign(metrics());
		}
		return myWordHeight;
	}
	...
	ZLPaintContext.Size getTextAreaSize() {
		return new ZLPaintContext.Size(getTextColumnWidth(),
				getTextAreaHeight());
	}
	...
	int getTextColumnWidth() {
		return twoColumnView() ? (getContextWidth() - getLeftMargin()
				- getSpaceBetweenColumns() - getRightMargin()) / 2
	...
	final void setTextStyle(ZLTextStyle style) {
		...
		// size = size+ ReadStyleManager.INS_FONT_SIZE_SP;
		// getContext().setFont(style.getFontEntries(), (int)size,
		// style.isBold(), style.isItalic(), style.isUnderline(),
		// style.isStrikeThrough());
		getContext().setFont(style.getFontEntries(),
				style.getFontSize(metrics()), style.isBold(), style.isItalic(),
				style.isUnderline(), style.isStrikeThrough());
	}

	final void resetTextStyle() {//复位style，就是设置style为assets/default/styles.css指定的style
		setTextStyle(getTextStyleCollection().getBaseStyle());
	}

	boolean isStyleChangeElement(ZLTextElement element) {//是否是StyleClose，ZLTextStyleElement，ZLTextControlElement中的一个
	...
	private void applyControl(ZLTextControlElement control) {
		if (control.IsStart) {
			final ZLTextHyperlink hyperlink = control instanceof ZLTextHyperlinkControlElement ? ((ZLTextHyperlinkControlElement) control).Hyperlink
					: null;
			final ZLTextNGStyleDescription description = getTextStyleCollection()
	...
	protected final ZLPaintContext.ScalingType getScalingType(
			ZLTextImageElement imageElement) {
		switch (getImageFitting()) {
		default:
		case none:
			return ZLPaintContext.ScalingType.IntegerCoefficient;
		case covers:
			return imageElement.IsCover ? ZLPaintContext.ScalingType.FitMaximum
	...
	public ZLPaintContext.Size getImageRectSize(){
		ZLPaintContext.Size realSize=null;
		ZLTextStyle style=getTextStyle();
		ZLPaintContext.Size temp = getTextAreaSize();
		int width=temp.Width-style.getLeftIndent(metrics())-style.getRightIndent(metrics());
		int height = temp.Height;
        realSize = new ZLPaintContext.Size(width, height);		
		return realSize;
	}
	/** 宽度测量之前，ZLPaintContext(具体实现类是ZLAndroidPaintContext)的画笔myTextPaint必须已经被设置好了，不同的字体样式测量的宽度值不一样 */
	final int getElementWidth(ZLTextElement element, int charIndex) {
		if (element instanceof ZLTextWord) {
			...
			ZLPaintContext.Size rect = getTextAreaSize();
			final ZLTextImageElement imageElement = (ZLTextImageElement) element;
			final ZLPaintContext.Size size = getContext().imageSize(
					imageElement.ImageData, getImageRectSize(),
			...
			return getContext().getSpaceWidth()
					* ((ZLTextFixedHSpaceElement) element).Length;
		}
		// add by yq
		else if (element instanceof ZLTextSpecialElement) {
			return getTextColumnWidth();
		}
		// end by yq
		return 0;
	}
	...
	final int getElementHeight(ZLTextElement element) {
		if (element instanceof ZLTextWord
			...
			return (size != null ? size.Height : 0)
					+ Math.max(
							getContext().getStringHeight()
									* (myTextStyle.getLineSpacePercent() - 100)
									/ 100, 3);
		} else if (element instanceof ZLTextVideoElement) {
			return Math.min(Math.min(200, getTextAreaHeight()),
					getTextColumnWidth() * 2 / 3);
		}
		// add by yq
		else if (element instanceof ZLTextSpecialElement) {
			return 12;
	...
	final int getWordWidth(ZLTextWord word, int start) {
		return start == 0 ? word.getWidth(getContext()) : getContext()
				.getStringWidth(word.Data, word.Offset + start,
						word.Length - start);
	}

	final int getWordWidth(ZLTextWord word, int start, int length) {
		return getContext().getStringWidth(word.Data, word.Offset + start,
				length);
	}
	...
	int getAreaLength(ZLTextParagraphCursor paragraph, ZLTextElementArea area,
			int toCharIndex) {
		setTextStyle(area.Style);
		final ZLTextWord word = (ZLTextWord) paragraph
				.getElement(area.ElementIndex);
		...
		if (length > 0) {
			return getWordWidth(word, area.CharIndex, length,
					selectHyphenationSign);
		}
		return 0;
	}

	final void drawWord(int x, int y, ZLTextWord word, int start, int length,
			boolean addHyphenationSign, ZLColor color) {
		final ZLPaintContext context = getContext();
		context.setTextColor(color);
		if (start == 0 && length == -1) {
			drawString(x, y, word.Data, word.Offset, word.Length,
					word.getMark(), 0);
		} else {
			if (length == -1) {
				length = word.Length - start;
			...
			if (!addHyphenationSign) {
				drawString(x, y, word.Data, word.Offset + start, length,
						word.getMark(), start);
			} else {
				char[] part = myWordPartArray;
				if (length + 1 > part.length) {
				...
				System.arraycopy(word.Data, word.Offset + start, part, 0,
						length);
				part[length] = '-';
	...
	private final void drawString(int x, int y, char[] str, int offset,
			int length, ZLTextWord.Mark mark, int shift) {
		final ZLPaintContext context = getContext();
		if (mark == null) {
			...
			for (; (mark != null) && (pos < length); mark = mark.getNext()) {
				...
				if (markStart > pos) {
					...
					x += context
							.getStringWidth(str, offset + pos, endPos - pos);
				}
				...
				if (markStart < length) {
					...
					final int endX = x
							+ context.getStringWidth(str, offset + markStart,
									endPos - markStart);
					context.fillRectangle(x, y - context.getStringHeight(),
							endX - 1, y + context.getDescent());
					context.drawString(x, y, str, offset + markStart, endPos
							- markStart);
					x = endX;
				}
