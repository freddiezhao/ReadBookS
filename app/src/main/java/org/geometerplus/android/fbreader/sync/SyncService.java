...
public class SyncService extends Service implements IBookCollection.Listener
{
	...
	{
		ZLog.d(ZLog.FBReaderSync, message);
	}
	...
	private enum Status
	{
		AlreadyUploaded(Book.SYNCHRONISED_LABEL), Uploaded(
				Book.SYNCHRONISED_LABEL), ToBeDeleted(Book.SYNC_DELETED_LABEL), Failure(
				Book.SYNC_FAILURE_LABEL), ServerError(null), SyncronizationDisabled(
				null), FailedPreviuousTime(null), HashNotComputed(null);

		private static final List<String>	AllLabels	= Arrays.asList(
																Book.SYNCHRONISED_LABEL, Book.SYNC_FAILURE_LABEL,
																Book.SYNC_DELETED_LABEL, Book.SYNC_TOSYNC_LABEL);
	...
	private final SyncNetworkContext	myBookUploadContext			= new SyncNetworkContext(
																			this, mySyncOptions, mySyncOptions.UploadAllBooks);
	private final SyncNetworkContext	mySyncPositionsContext		= new SyncNetworkContext(
																			this, mySyncOptions, mySyncOptions.Positions);

	private static volatile Thread		ourSynchronizationThread;
	private static volatile Thread		ourQuickSynchronizationThread;

	private final List<Book>			myQueue						= Collections
																			.synchronizedList(new LinkedList<Book>());
	...
	{
		return PendingIntent.getService(this, 0, new Intent(this, getClass())
				.setAction(SyncOperations.Action.SYNC), 0);
	}
		...
		if (SyncOperations.Action.START.equals(action)) {
			...
			config.runOnConnect(new Runnable()
			{
				public void run()
				{
					...
					myCollection.bindToService(SyncService.this,
							myQuickSynchroniser);
				}
			});
	...
	private synchronized void initHashTables()
	{
		...
		try {
			...
			log(String.format("RECEIVED: %s/%s HASHES",
					myActualHashesFromServer.size(),
					myDeletedHashesFromServer.size()));
															...
															if (ourSynchronizationThread == null) {
																ourSynchronizationThread = new Thread()
																{
																	public void run()
																	{
																		...
																		try {
																			clearHashTables();
																			for (BookQuery q = new BookQuery(
																					new Filter.Empty(), 20);; q = q.next()) {
																			...
																			while (!myQueue.isEmpty()) {
																				...
																				statusCounts.put(status, sc != null ? sc + 1
																						: 1);
																			}
																		} finally {
																			log("SYNCHRONIZATION FINISHED IN "
	...
	private final class UploadRequest extends ZLNetworkRequest.FileUpload
	{
		...
		@Override
		public void handleStream(InputStream stream, int length)
				throws IOException, ZLNetworkException
		{
			final Object response = JSONValue.parse(new InputStreamReader(
					stream));
			String id = null;
			...
			try {
				...
				if (responseList.size() == 1) {
					final Map resultMap = (Map) responseList.get(0).get(
							"result");
	...
	private Status uploadBookToServerInternal(Book book)
	{
		...
		final PostRequest verificationRequest = new PostRequest(
				"book.status.by.hash", Collections.singletonMap("sha1", hash))
		{
			@Override
			public void processResponse(Object response)
			{
		...
		final String csrfToken = myBookUploadContext.getCookieValue(
				SyncOptions.DOMAIN, "csrftoken");
		try {
			final String status = (String) result.get("status");
			if ((force && !"found".equals(status))
					|| "not found".equals(status)) {
				try {
					final UploadRequest uploadRequest = new UploadRequest(file);
					uploadRequest.addHeader("Referer",
							verificationRequest.getURL());
	...
	private void syncPositions()
	{
		try {
			mySyncPositionsContext.perform(new JsonRequest2(
					SyncOptions.BASE_URL + "sync/position.exchange", mySyncData
							.data(myCollection))
			{
				@Override
				public void processResponse(Object response)
				{
					if (mySyncData
							.updateFromServer((Map<String, Object>) response)) {
